...
class KnowledgeBase:
    ...
    def __init__(self):
        ...
        if not self.emb_config.api_key or self.emb_config.api_key == "empty":
            print(
                "Warning: Embedding API Key not configured. Knowledge base search might fail."
            )
        ...
        self.collection = self.client.get_or_create_collection(
            name="excel_knowledge", metadata={"hnsw:space": "cosine"}
        )
    ...
    def _create_item_with_metadata(
        self, metadata: dict, body: str, source: Path
    ) -> KnowledgeItem:
        """使用显式元数据创建知识条目"""
        # 生成 ID（优先使用元数据中的 ID）
    ...
    def _create_item_auto(self, content: str, source: Path) -> KnowledgeItem:
        ...
        return KnowledgeItem(
            id=item_id, content=content, title=title, tags=tags, source_file=str(source)
        )

    @lru_cache(maxsize=100)
    def search(
        self, query: str, columns: Tuple[str] = None, top_k: int = None
    ) -> List[KnowledgeItem]:
        """检索相关知识 (使用 LRU 缓存)"""
        # columns 需要转换为 tuple 才能 hash
        ...
        for i, doc in enumerate(results["documents"][0]):
            ...
            items.append(
                KnowledgeItem(
                    ...
                    related_columns=(
                        meta.get("related_columns", "").split(",")
                        if meta.get("related_columns")
                        else []
                    ),
            ...
            metadatas=[
                {
                    ...
                    "related_columns": (
                        ",".join(item.related_columns) if item.related_columns else ""
                    ),
    ...
    def update_entry(
        ...
        new_meta = {
            ...
            "category": (
                ...
                else old_meta.get("category", "general")
            ),
